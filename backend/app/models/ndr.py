...
class NDRCase(Base):
    ...
    tracking_enrolled_at = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    tracking_next_attempt_at = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    tracking_last_attempted_at = mapped_column(DateTime(timezone=True), nullable=True)
    tracking_expires_at = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    tracking_attempt_count = mapped_column(Integer, nullable=False, default=0)
    tracking_consecutive_failures = mapped_column(Integer, nullable=False, default=0)
    tracking_last_result = mapped_column(String(32), nullable=True)
    tracking_classification = mapped_column(String(32), nullable=True, index=True)
    tracking_classified_at = mapped_column(DateTime(timezone=True), nullable=True)
