...
def _delivered(shipment: ShiprocketShipment) -> bool:
    return any(str(value or "").strip().casefold() == "delivered" for value in (shipment.normalized_status, shipment.terminal_status, shipment.latest_status))
...
def resolve_if_canonically_delivered(db: Session, case: NDRCase, *, now: datetime | None = None) -> bool:
    shipment = canonical_shipment_for_case(db, case)
    if shipment is None or not _delivered(shipment):
        return False
    ...
    case.resolution_note = case.resolution_note or "Resolved automatically after canonical shipment delivery was confirmed."
    db.add(NDREvent(id=str(uuid4()), case_id=case.id, event_type="delivered_resolution", description="NDR closed after canonical shipment delivery was confirmed.", actor_name="Mumchies OS", event_data={"shipment_order_id": shipment.order_id, "awb": shipment.awb}))
    return True


def resolve_active_delivered_cases(db: Session) -> int:
    resolved = 0
    for case in db.scalars(select(NDRCase).where(NDRCase.source_lifecycle == "active", NDRCase.current_status != "resolved")).all():
        if resolve_if_canonically_delivered(db, case):
            resolved += 1
    if resolved:
        db.commit()
    return resolved
