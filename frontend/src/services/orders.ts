...
export async function saveManualShadowfaxShipment(orderId: string, payload: { awb?: string; service_name?: string; booked_at?: string; freight?: number; note?: string }): Promise<{ provider: string; shipment: Order['shipment']; warning?: string; provider_warning?: string; validation_source?: string; message?: string; shiprocket_cleanup?: { status: string; error?: string } }> {
  ...
  const body = await response.json().catch(() => null)
  if (!response.ok) throw new Error(readableApiError(body, 'Unable to reconcile the Shadowfax shipment. Please retry.'))
  return body
}
...
export type ShiprocketSearchDiagnostic = Record<string, unknown>

export async function inspectShiprocketSearch(orderNumber: string): Promise<ShiprocketSearchDiagnostic> {
  const response = await apiFetch(`${apiBase}/api/v1/couriers/shiprocket/debug-search?order_number=${encodeURIComponent(orderNumber)}`)
  const body = await response.json().catch(() => null)
  if (!response.ok) throw new Error(readableApiError(body, 'Could not inspect the Shiprocket search response.'))
