...
import { getGstReport, gstReportDownloadUrl, type GstReport } from '../services/reports'
...
function GstReportPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const generate = async (refresh = false) => {
    ...
    try { setReport(await getGstReport(month, refresh)) }
  ...
  return <div>
    ...
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><p className="text-sm font-medium text-[#ff6b35]">Reports / GST</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Monthly GST report</h2><p className="mt-1 text-sm text-slate-500">Online sales selected by actual Shopify delivery date.</p></div>
      <div className="flex flex-wrap items-end gap-2">
        <label><span className="mb-1 block text-xs font-semibold text-slate-500">Month</span><input aria-label="Report month" type="month" max={currentMonth()} value={month} onChange={event => setMonth(event.target.value)} className="rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm" /></label>
        <button disabled={loading || !month} onClick={() => void generate()} className="rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{loading ? 'Generating…' : 'Generate Report'}</button>
        <button disabled={loading || !month} onClick={() => void generate(true)} className="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 disabled:opacity-50">Refresh Data</button>
        <a aria-disabled={!report || report.summary.exceptions > 0} title={report?.summary.exceptions ? 'Resolve exceptions before downloading.' : undefined} onClick={event => { if (!report || report.summary.exceptions > 0) event.preventDefault() }} href={report && report.summary.exceptions === 0 ? gstReportDownloadUrl(report.month) : '#'} className={`rounded-lg border px-4 py-2.5 text-sm font-semibold ${report && report.summary.exceptions === 0 ? 'border-slate-200 bg-white text-slate-700' : 'pointer-events-none border-slate-100 bg-slate-50 text-slate-300'}`}>Download CSV</a>
      </div>
    </div>
