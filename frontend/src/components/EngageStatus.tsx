    ...
    <div className="flex items-start">
      {stages.map((stage, index) => <div key={stage.name} className="contents">
        <div className="flex w-24 shrink-0 flex-col items-center text-center">
          <span title={stage.message || ''} aria-label={`${stage.name}: ${stage.message || ''}`} className={`inline-flex h-9 w-9 items-center justify-center rounded-full text-[10px] font-bold ${engageStyle(stage.value)}`}>{stage.abbreviation}</span>
