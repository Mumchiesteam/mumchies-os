...
export function UsersPage() {
  ...
  return <div>
    ...
    {showCreate && <Modal title="Create User" onClose={() => setShowCreate(false)}><form onSubmit={submitCreate}><label className="block text-sm">Username<input name="username" autoComplete="off" required maxLength={64} className={inputClass} /></label><label className="mt-3 block text-sm">Display Name<input name="display_name" autoComplete="name" required maxLength={120} className={inputClass} /></label><label className="mt-3 block text-sm">Role<select name="role" defaultValue="operator" className={inputClass}><option value="admin">Admin</option><option value="operator">Operator</option></select></label><label className="mt-3 block text-sm">Password<input name="password" autoComplete="new-password" type="password" minLength={6} required className={inputClass} /></label><p className="mt-1 text-xs text-slate-500">Use at least 6 characters. Numbers-only passwords are allowed.</p><label className="mt-3 block text-sm">Confirm Password<input name="password_confirmation" autoComplete="new-password" type="password" minLength={6} required className={inputClass} /></label><Actions busy={busy} onCancel={() => setShowCreate(false)} submitLabel="Create User" /></form></Modal>}
    {resetId !== null && <Modal title="Reset Password" onClose={() => setResetId(null)}><form onSubmit={submitReset}><label className="block text-sm">New Password<input name="password" autoComplete="new-password" type="password" minLength={6} required className={inputClass} /></label><p className="mt-1 text-xs text-slate-500">Use at least 6 characters. Numbers-only passwords are allowed.</p><label className="mt-3 block text-sm">Confirm Password<input name="password_confirmation" autoComplete="new-password" type="password" minLength={6} required className={inputClass} /></label><Actions busy={busy} onCancel={() => setResetId(null)} submitLabel="Reset Password" /></form></Modal>}
  </div>
}
